...
export const Button = ({ type, handler, closeModal,disabled, pageNum, bg_color, text_color, name }: button) => {
  ...
  switch (type) {
        case "modal":
            return <button type="button" className={bg_color +" py-3 px-4 my-1 text-xs w-full  font-normal rounded-md cursor-pointer focus:outline-none "+text_color} onClick={() => handler ? handler() : closeModal()}> {name} </button>;
...
// export const Button = ({ btype, handler, closeModal,disabled, pageNum, bg_color, text_color, name }: button) => {
//    const buttonType = btype.trim().toLowerCase();
