...
export const PotEditModal = ({ isOpen, closeModal, modalType , id }: any) => {
	...
	const handlerSubmit = (e: React.FormEvent<HTMLFormElement>) => {
		...
		const newPot: pot = {id, name,target,total: 0,theme};
			...
			<div className="fixed inset-0 z-50 flex justify-center items-center bg-opacity-100">
					...
					<div className="bg-white p-6 rounded-xl shadow-xl max-w-sm w-full relative">
						...
						<div className="pb-3">
							...
							<input type="text" name="name"  value={pot?.name} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
							<p className="text-xs text-right text-gray-400 pt-1">30 characters left</p>
						</div>
						<div className="pb-3">
							<label className="block text-sm font-medium text-gray-700">Target</label>
							<input type="text" name="target" value={pot?.target} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
						</div>
						<div className="pb-5">
						...
						<button type="submit" className="text-xs w-full py-3 px-4 bg-black text-white font-normal rounded-md focus:outline-none">Save Chages</button>
					</div>
				</form>
			</div>
		);
